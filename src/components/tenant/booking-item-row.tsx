...
/**
 * One \"cart line\" in the booking form — its own item, dates, discount and
 * live availability/price preview. Quote state is reported up to
 * `BookingForm` (via `onQuoteChange`) so the order summary panel can show
 * one aggregate total instead of a separate price card per row.
 */
export function BookingItemRow({
  ...
  }, [selectedItemId, fromDate, toDate, quantity, discountAmount, itemKey]);
